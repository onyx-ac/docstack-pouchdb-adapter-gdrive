...
- Also deletes the **previous** `snapshot-data-*.json` file (not just the previous index) once the new snapshot is committed, so a superseded data chunk doesn't linger as unreferenced clutter. See `docs/TESTING.md` for the tool that verifies this.
